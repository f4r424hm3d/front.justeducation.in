...
const SchoolDetailsPage: React.FC = () => {
  ...
  const school = {
    ...
    type: "Private",
    description:
      "Delhi Public School is one of the most prestigious schools in India, known for its academic excellence, holistic development, and world-class facilities. The school offers a nurturing environment for students from kindergarten to grade 12.\n\nFor 12th class admissions, students can choose from Science, Commerce, and Arts streams, each with specific eligibility and requirements.",

    // Add a hero background image
    heroBackground:
      "https://edunext-main-storage-cf.edunexttechnologies.com/dpsdehradun/school___static/1690881981141_slider01.jpg?auto=format&fit=crop&w=1600&q=80",
  };
  ...
  const mediaGallery: MediaItem[] = [
    ...
    {
      type: "image",
      src: "https://images.unsplash.com/photo-1547891654-e66ed7ebb968?auto=format&fit=crop&w=400&q=80",
      alt: "Sports Ground",
    },
    {
      type: "image",
      src: "https://images.unsplash.com/photo-1588075592446-265fd1e6e76f?auto=format&fit=crop&w=400&q=80",
      alt: "Laboratory",
    },
    {
      type: "video",
  ...
  const [selectedSpecializationCourse, setSelectedSpecializationCourse] =
  ...
  const handleVisitScheduleFormChange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>
  ) => {
    const { name, value } = e.target;
  ...
  const [openFeeDetails, setOpenFeeDetails] = useState<{
    [key: string]: boolean;
  }>({});
  ...
  const [organizationFacilities, setOrganizationFacilities] = useState<
    OrganizationFacility[]
  >([]);
  const [organizationFacilitiesLoading, setOrganizationFacilitiesLoading] =
    useState(true);
  ...
  >([]);
  const [organizationOverviewLoading, setOrganizationOverviewLoading] =
    useState(true);
  const [organizationAdmissions, setOrganizationAdmissions] = useState<
    OrganizationAdmission[]
  >([]);
  const [organizationAdmissionsLoading, setOrganizationAdmissionsLoading] =
    useState(true);

  const [organizationFeeStructure, setOrganizationFeeStructure] = useState<
    OrganizationFeeStructure[]
  >([]);
  const [organizationFeeStructureLoading, setOrganizationFeeStructureLoading] =
    useState(true);

  const [organizationAlumnis, setOrganizationAlumnis] = useState<
    OrganizationAlumni[]
  >([]);
  const [organizationAlumnisLoading, setOrganizationAlumnisLoading] =
    useState(true);
  ...
  const [organizationCareers, setOrganizationCareers] = useState<
    OrganizationCareer[]
  >([]);
  const [organizationCareersLoading, setOrganizationCareersLoading] =
    useState(true);

  const [organizationContacts, setOrganizationContacts] = useState<
    OrganizationContact[]
  >([]);
  const [organizationContactsLoading, setOrganizationContactsLoading] =
    useState(true);

  const [organizationCurricula, setOrganizationCurricula] = useState<
    OrganizationCurriculum[]
  >([]);
  const [organizationCurriculaLoading, setOrganizationCurriculaLoading] =
    useState(true);

  const [organizationPhotos, setOrganizationPhotos] = useState<
    OrganizationPhotoCategory[]
  >([]);
  const [organizationPhotosLoading, setOrganizationPhotosLoading] =
    useState(true);

  const [organizationFeaturedPosts, setOrganizationFeaturedPosts] = useState<
    OrganizationFeaturedPost[]
  >([]);
  const [
    organizationFeaturedPostsLoading,
    setOrganizationFeaturedPostsLoading,
  ] = useState(true);
  ...
  const [organizationVideos, setOrganizationVideos] = useState<
    OrganizationVideo[]
  >([]);
  const [organizationVideosLoading, setOrganizationVideosLoading] =
    useState(true);

  const [organizationTimings, setOrganizationTimings] = useState<
    OrganizationTiming[]
  >([]);
  const [organizationTimingsLoading, setOrganizationTimingsLoading] =
    useState(true);

  const [organizationFeaturedPhotos, setOrganizationFeaturedPhotos] = useState<
    OrganizationFeaturedPhoto[]
  >([]);
  const [
    organizationFeaturedPhotosLoading,
    setOrganizationFeaturedPhotosLoading,
  ] = useState(true);

  const [organizationPrograms, setOrganizationPrograms] = useState<
    OrganizationProgram[]
  >([]);
  const [organizationProgramsLoading, setOrganizationProgramsLoading] =
    useState(true);
  ...
  const [downloadType, setDownloadType] = useState<"brochure" | "fees" | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const fetchSchoolDetails = async () => {
      try {
        const response = await api.get<ApiResponse<schoolDetails>>(
          `/organization-details-by-slug/${slug}`
        );
  ...
  useEffect(() => {
    ...
    const fetchEducationBoard = async () => {
      try {
        ...
        const res =
          await api.get<ApiResponse<EducationBoard[]>>("/education-boards");
        console.log(res.data.data);
  ...
  useEffect(() => {
    ...
    const fetchSchoolClasses = async () => {
      try {
        ...
        const res =
          await api.get<ApiResponse<SchoolClass[]>>("/school-classes");
        console.log(res.data.data);
  ...
  useEffect(() => {
    ...
    const fetchOrganizationOverview = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationOverviewResponse>(
            `/organization-overviews/${schoolDetails.id}`
          );
  ...
  useEffect(() => {
    ...
    const fetchOrganizationAdmissions = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationAdmissionResponse>(
            `/organization-admissions/${schoolDetails.id}`
          );
  ...
  useEffect(() => {
    ...
    const fetchOrganizationFeeStructure = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationFeeStructureResponse>(
            `/organization-fee-structure/${schoolDetails.id}`
          );
    ...
    const fetchOrganizationAlumnis = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationAlumniResponse>(
            `/organization-alumnis/${schoolDetails.id}`
          );
    ...
    const fetchOrganizationNews = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationNewsResponse>(
            `/organization-news/${schoolDetails.id}`
          );
    ...
    const fetchOrganizationCareers = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationCareerResponse>(
            `/organization-careers/${schoolDetails.id}`
          );
    ...
    const fetchOrganizationContacts = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationContactResponse>(
            `/organization-contacts/${schoolDetails.id}`
          );
    ...
    const fetchOrganizationCurricula = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationCurriculumResponse>(
            `/organization-curricula/${schoolDetails.id}`
          );
    ...
    const fetchOrganizationPhotos = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationPhotosResponse>(
            `/organization-photos/${schoolDetails.id}`
          );
    ...
    const fetchOrganizationFeaturedPosts = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationFeaturedPostResponse>(
            `/organization-featured-posts/${schoolDetails.id}`
          );
    ...
    const fetchOrganizationFAQs = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationFAQResponse>(
            `/organization-faqs/${schoolDetails.id}`
          );
    ...
    const fetchOrganizationFacilities = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationFacilityResponse>(
            `/organization-facilities/${schoolDetails.id}`
          );
    ...
    const fetchOrganizationVideos = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationVideosResponse>(
            `/organization-videos/${schoolDetails.id}`
          );
    ...
    const fetchOrganizationTimings = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationTimingsResponse>(
            `/organization-timings/${schoolDetails.id}`
          );
    ...
    const fetchOrganizationFeaturedPhotos = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationFeaturedPhotosResponse>(
            `/organization-featured-photos/${schoolDetails.id}`
          );
    ...
    const fetchOrganizationPrograms = async () => {
      try {
        ...
        if (schoolDetails?.id) {
          const res = await api.get<OrganizationProgramsResponse>(
            `/organization-programs/${schoolDetails.id}`
          );
  ...
  return (
    <MainLayout>
      <div className="min-h-screen bg-gray-50">
              ...
              <div className="flex items-start gap-4 mb-6">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-gray-600">
                    ...
                    <span>
                      {schoolDetails?.city}, {schoolDetails?.country || "India"}
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4 w-full">
                  <div className="flex items-center gap-2 text-gray-600 bg-gray-100 px-3 py-2 rounded-lg">
                    ...
                    <span className="text-sm">
                      Estd. {schoolDetails?.established || school.established}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 text-gray-600 bg-gray-100 px-3 py-2 rounded-lg">
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-6 mb-4">
                    <div className="flex items-center gap-2 text-gray-600">
                      ...
                      <span>
                        {schoolDetails?.established || school.established}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-gray-600">
                      <FaGraduationCap className="text-blue-600" />
        ...
        <div className="sticky top-[60px] z-50 bg-white border-b shadow-sm">
          <div className="relative w-full">
            ...
            <div
              className="flex overflow-x-auto pb-2 pt-2 px-6 hide-scrollbar"
              ref={tabsScrollRef}
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
            ...
            <div className="lg:col-span-2 space-y-3 sm:space-y-4 lg:space-y-6">
                              ...
                              >
                                ...
                                <td className="px-4 py-3">
                                  <button
                                    ...
                                    onClick={() =>
                                      handleApplyClick(program.program_name)
                                    }
                                  >
                                    Apply
                                  </button>
                ...
                {activeTab === "Admissions" && (
                  <div className="bg-white rounded-lg sm:p-6 mb-4">
                    ...
                    {organizationAdmissionsLoading ? (
                      ...
                      <div className="text-center py-8">
                        <p className="text-gray-500">
                          No admission information available at the moment.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  {organizationFeeStructureLoading ? (
                    ...
                    <div className="space-y-2">
                      {organizationFeeStructure.map((item) => (
                        <div
                          key={item.id}
                          className="bg-white border rounded px-4 py-3 text-sm font-medium shadow-sm"
                        >
                          <div
                            className="flex items-center justify-between cursor-pointer"
                            onClick={() => toggleFeeDetails(item.class)}
                          >
                            <div>
                            ...
                            <div className="flex items-center gap-2">
                              <span className="font-semibold text-gray-800">
                                ₹
                                {parseInt(item.total_fee).toLocaleString(
                                  "en-IN"
                                )}
                              </span>
                              <svg
                                ...
                                viewBox="0 0 24 24"
                                stroke="currentColor"
                                className={`text-gray-400 transition-transform duration-200 ${openFeeDetails[item.class] ? "rotate-180" : ""}`}
                              >
                                <path
                                  strokeLinecap="round"
                                  strokeLinejoin="round"
                          ...
                          {openFeeDetails[item.class] && (
                            <div className="mt-3 pt-3 border-t border-gray-200 text-gray-700 space-y-1 text-xs">
                              <div>
                                Admission Fee: ₹
                                {parseInt(item.admission_fee).toLocaleString(
                                  "en-IN"
                                )}
                              </div>
                              <div>
                                Tuition Fee: ₹
                                {parseInt(item.tuition_fee).toLocaleString(
                                  "en-IN"
                                )}
                              </div>
                              <div>
                    ...
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p>
                        No fee structure information available at the moment.
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div>
                  <div className="mb-4">
                    ...
                    <p className="text-sm font-bold text-black-600">
                      Meet our successful graduates who are making a difference
                      in their fields.
                    </p>
                  </div>
                          ...
                          <div className="flex overflow-x-auto gap-2 pb-1 mt-1">
                            {category.photos.map((photo, photoIndex) => (
                              <img
                                ...
                                onClick={() =>
                                  openMediaModal(
                                    categoryIndex * 100 + photoIndex
                                  )
                                }
                                onError={(e) => {
                                  const target = e.target as HTMLImageElement;
                                  target.src =
                                    "https://via.placeholder.com/96x96/cccccc/666666?text=Photo";
                                }}
                              />
                            ))}
                          </div>
              ...
              {activeTab === "Reviews" && (
                <div className="flex flex-col items-center justify-center py-6">
                  ...
                  <form
                    ...
                    className="w-full bg-white rounded shadow p-6 mb-8"
                  >
                    ...
                    <div className="mb-4">
                      ...
                      <textarea
                        name="overall"
                        value={reviewInput.overall}
                        onChange={handleReviewInput}
                        className="w-full border rounded p-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 min-h-[80px] resize-y"
                        required
                      />
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                      {[
                        { key: "placement", label: "Academics" },
                        { key: "infrastructure", label: "Infrastructure" },
                        { key: "faculty", label: "Faculty" },
                        { key: "hostel", label: "Campus Life" },
                      ].map(({ key, label }) => (
                              ...
                              >
                                <FaStar
                                  className={
                                    Number(
                                      reviewInput[
                                        key as keyof typeof reviewInput
                                      ]
                                    ) >= star
                                      ? "text-orange-400"
                                      : "text-gray-300"
                    ...
                    <div className="pt-2 flex justify-center">
                      <button
                        type="submit"
                        className="mt-3 bg-blue-600 text-white px-8 py-2 rounded font-semibold hover:bg-blue-700 transition text-sm"
                      >
                        Submit
                      </button>
                    </div>
                  ...
                  <div className="w-full space-y-8">
                    {reviews.length === 0 ? (
                      ...
                      reviews.map((review, idx) => (
                        <div
                          key={idx}
                          className="bg-white border rounded-lg shadow p-6 flex flex-col relative"
                        >
                          {/* Rating badge */}
                  ...
                  {organizationFeaturedPostsLoading ? (
                    ...
                    <div className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
                      <div className="text-center py-8">
                        ...
                        <p className="text-gray-500">
                          There are currently no featured posts for this
                          organization.
                        </p>
                      </div>
                    </div>
                      ...
                      <>
                        ...
                        <div className="space-y-6 mb-8">
                          {organizationCareers.map((career, index) => (
                            <div
                              key={career.id}
                              className="bg-green-50 p-6 rounded-lg hover:shadow-md transition-shadow border border-green-100"
                            >
                              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                                <div className="flex-1">
                                  <div className="flex items-center gap-3 mb-3">
                                    <span className="font-bold text-green-800 min-w-[30px]">
                                      {index + 1}.
                                    </span>
                      ...
                      <div className="space-y-6">
                        {organizationNews.map((news, index) => {
                          ...
                          return (
                            <div
                              key={news.id}
                              className={`bg-${colorClass}-50 rounded-lg p-6`}
                            >
                              <div className="flex items-start justify-between mb-3">
                                <h3
                                  className={`text-lg font-bold text-${colorClass}-800`}
                                >
                                  {news.title}
                                </h3>
              ...
              {activeTab === "Alumni" && (
                <div className="mt-4 sm:mt-6">
                  ...
                  <div className="mb-8">
                    <h2 className="text-lg sm:text-xl text-blue-600 font-semibold mb-4">
                      Success Rates / Previous Year Results
                    </h2>
                    <p className="text-gray-600 mb-6 text-sm sm:text-base">
                      Our students consistently deliver outstanding results
                      across boards and competitive exams. Here's a quick
                      snapshot from the previous year.
                    </p>
                ...
                <div className="bg-white rounded-lg shadow-md border p-6">
                  <div className="text-center mb-8">
                    ...
                    <p className="text-gray-600">
                      Everything you need to know about Institution.
                    </p>
                  </div>
            ...
            <div className="space-y-3 sm:space-y-4 lg:space-y-6 mt-4 lg:mt-0">
              ...
              <div className="bg-blue-900 text-white rounded-lg shadow-md p-4 w-full">
                ...
                <div className="flex gap-3 mb-3">
                  <button
                    className="flex-1 bg-orange-500 text-white font-semibold py-2 rounded-md shadow-sm hover:bg-orange-600 transition"
                ...
                <div className="text-center text-gray-300 text-sm mt-3">
                  <span className="bg-blue-800 px-2 py-1 rounded mr-1 text-white font-semibold">
                    12 people
                  </span>{" "}
                  recently enquired
                </div>
                    ...
                    {galleryImages
                      .slice(
                        ...
                        expandedGallery
                          ...
                          : Math.min(4, galleryImages.length)
                      )
                      .map((image, index) => {
              ...
              <div className="bg-white rounded-lg shadow p-3 sm:p-4 mb-4">
                <h2 className="text-lg sm:text-xl font-semibold mb-2 sm:mb-3">
                  Time of Institution Open (Day Wise)
                </h2>
                {organizationTimingsLoading ? (
                    ...
                    <table className="min-w-full text-xs sm:text-sm border rounded-lg overflow-hidden">
                      <thead>
                        <tr className="bg-blue-50 text-gray-700">
                          <th className="px-2 sm:px-3 py-2 font-bold text-left">
                            Day
                          </th>
                          <th className="px-2 sm:px-3 py-2 font-bold text-left">
                            Open Time
                  ...
                  <div className="text-center py-8">
                    <p className="text-gray-500">
                      No timing information available at the moment.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white rounded-lg shadow p-3 sm:p-4 mb-4">
                ...
                <div className="flex flex-wrap gap-6 sm:gap-10 items-center">
                  <div className="flex flex-col items-center">
                    <div className="w-10 h-10 rounded-full bg-green-500 flex items-center justify-center text-white text-2xl font-bold mb-1">
                      A
                    </div>
                  ...
                  <div className="flex flex-col items-center">
                    <div className="w-10 h-10 rounded-full bg-green-600 flex items-center justify-center text-white text-2xl font-bold mb-1">
                      अ
                    </div>
                    <span className="text-base font-medium text-gray-700">
                      Hindi
                  ...
                  <div className="flex flex-col items-center">
                    <div className="w-10 h-10 rounded-full bg-green-400 flex items-center justify-center text-white text-2xl font-bold mb-1">
                      Ê
                    </div>
                    <span className="text-base font-medium text-gray-700">
                      French
                  ...
                  <div className="flex flex-col items-center">
                    <div className="w-10 h-10 rounded-full bg-green-700 flex items-center justify-center text-white text-2xl font-bold mb-1">
                      सं
                    </div>
                    <span className="text-base font-medium text-gray-700">
                      Sanskrit
              ...
              <div className="bg-white rounded-lg shadow p-3 sm:p-4 mb-4">
                ...
                <div className="flex flex-wrap gap-8 items-center mb-4">
                  ...
                  <div className="flex flex-col items-center opacity-60">
                    <div className="w-14 h-14 rounded-lg bg-gray-100 flex items-center justify-center mb-2">
                      <img
                        src="https://img.icons8.com/ios-filled/50/cccccc/steak.png"
                        alt="Non Vegetarian"
                        className="w-10 h-10"
                      />
                    </div>
                ...
                <div className="text-gray-500 text-sm mt-2">
                  Student menu is designed in consultation with the nutritionist
                  and dining hall committee, which consist of students and staff
                  as well. We serve five meals a day which includes breakfast,
                  the juice break, lunch, teatime snack, dinner, as well as milk
        ...
        {showApplyModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div
              className="bg-white p-3 rounded-md max-w-xs w-full relative"
              style={{ minWidth: 0 }}
            >
              <button
              ...
              <form onSubmit={handleApplyFormSubmit} className="space-y-3">
                ...
                <div className="pt-2 flex justify-center">
                  <button
                    type="submit"
                    className="bg-blue-600 text-white py-2 px-8 rounded font-semibold hover:bg-blue-700 transition text-sm"
                  >
                    Submit
                  </button>
                </div>
        ...
        {showVisitScheduleModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-4 rounded-lg max-w-md w-full relative shadow-lg">
              ...
              <form
                onSubmit={handleVisitScheduleFormSubmit}
                className="space-y-3"
              >
                {/* School Name (auto-filled) */}
                <div className="relative">
            ...
            <div className="bg-white p-5 rounded-xl max-w-2xl w-[90%] relative shadow-2xl border border-gray-100">
              ...
              <div className="max-h-[70vh] overflow-y-auto px-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {!selectedCompareSchool ? (
                    <div
                      className="border rounded-lg p-3 shadow-sm bg-gray-50 cursor-pointer flex items-center"
                    ...
                    <div className="border rounded-lg p-3 shadow-sm bg-green-50">
                      <div className="flex items-center">
                        <div className="relative w-12 h-12 bg-white rounded-full flex items-center justify-center border shadow-sm mr-3">
                          ...
                          <button
                            onClick={() => setSelectedCompareSchool(null)}
                      ...
                      <div className="mt-3 space-y-2 text-xs border-t pt-3">
                        <div className="flex justify-between">
                          <span className="text-gray-600">Established:</span>
                          <span className="font-medium">
                            {selectedCompareSchool.established}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Board:</span>
                          <span className="font-medium">
                            {selectedCompareSchool.board}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Classes:</span>
                          <span className="font-medium">
                            {selectedCompareSchool.classes}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Rating:</span>
              ...
              <div className="flex items-center justify-between p-4 border-b">
                <div>
                  ...
                  <p className="text-gray-600 mt-1 text-sm">
                    ...
                    Students have shown interest in the last 30 days
                  </p>
                </div>
                <button
                ...
                <div className="p-4">
                  <div className="overflow-x-auto">
                    <table className="min-w-full bg-white border border-gray-200 rounded-lg">
                      ...
                      <tbody>
                        {courseData[
                          selectedCourse as keyof typeof courseData
                  ...
                  return dynamicGallery[currentMediaIndex]?.type === "video" ? (
                    ...
                    <div className="relative w-full max-w-4xl">
                      <img
                        src={dynamicGallery[currentMediaIndex]?.src}
                        alt={
                          dynamicGallery[currentMediaIndex]?.alt ||
                          "Gallery Image"
                        }
                        ...
                        {item.type === "video" ? (
                          ...
                          <img
                            ...
                            onError={(e) => {
                              const target = e.target as HTMLImageElement;
                              target.src =
                                "https://via.placeholder.com/64x48/cccccc/666666?text=Photo";
                            }}
                          />
                        )}
                      </div>
                      ...
                      <div className="flex items-center border border-gray-300 rounded-lg bg-white h-10">
                        ...
                        <div className="flex-1 flex items-center px-3">
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              document.getElementById("job-resume")?.click()
                            }
                          ...
                          <span className="ml-2 text-xs text-gray-500">
                            {jobApplicationForm.resume
                              ? jobApplicationForm.resume.name
                              : "No file chosen"}
                          </span>
                        </div>
                      </div>
